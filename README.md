...
저장소에 포함된 실행에서는 최종 생존 사례 12건, 반려 기록 14건이 남았습니다. 반려 기록은 재생성 시도마다 따로 남깁니다. 한 번 이상 반려된 사례는 8건이고, 이 중 5건은 재생성 후 통과했으며 3건은 재생성 한도를 넘어 폐기되었습니다. 반려 기록 14건 중 11건(79%)은 반려 사유에 인용 문제가 포함되어 있습니다.

6개 판단 경계 축은 모두 채웠지만 정상사례와 반례가 각각 0 / 2로 목표에 미치지 못해 최종 상태는 `INSUFFICIENT_COVERAGE`이고, Test-case Formatter는 차단되었습니다. Coverage 조건을 충족하지 못하면 Test Case를 만들지 않도록 한 gate가 그대로 동작한 결과입니다.

정상사례와 반례가 생성되지 않는 것은 현재 실행의 한계이며, [9. 개발 과정 및 트러블슈팅](#9-개발-과정-및-트러블슈팅)의 후속 개선 과제로 남겨 두었습니다.
